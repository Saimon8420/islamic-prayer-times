...
const PrayerIcons = {
  ...
  Sunrise: () => (
    <svg viewBox="0 0 24 24" fill="none" className="w-6 h-6">
      ...
      <path d="M12 2v2M12 20v2M4.93 4.93l1.41 1.41M17.66 6.34l1.41-1.41M2 12h2M20 12h2M6.34 17.66l-1.41 1.41M17.66 17.66l1.41 1.41" stroke="currentColor" strokeWidth="2" strokeLinecap="round" />
    </svg>
  ),
  Dhuhr: () => (
    <svg viewBox="0 0 24 24" fill="none" className="w-6 h-6">
  ...
  Isha: () => (
    <svg viewBox="0 0 24 24" fill="none" className="w-6 h-6">
      <path d="M21 12.79A9 9 0 1 1 11.21 3 7 7 0 0 0 21 12.79z" fill="currentColor" fillOpacity="0.3" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
...
export const PrayerTimesCard = () => {
  ...
  const mainPrayers = ['Fajr', 'Sunrise', 'Dhuhr', 'Asr', 'Maghrib', 'Isha'] as const;
  ...
  const calculateProgress = () => {
    ...
    const prayerIndex = mainPrayers.indexOf(nextPrayer.name as (typeof mainPrayers)[number]);
    if (prayerIndex <= 0) return 0;
    const prevPrayerName = mainPrayers[prayerIndex - 1];
    const prevTime = prayerTimesMap[prevPrayerName];
  ...
  const methodName = CALCULATION_METHODS.find(m => m.id === calculationMethod)?.name || 'Standard Method';
    ...
    <Card className="islamic-border overflow-hidden fade-in">
      ...
      <div className="islamic-gradient p-6 text-white relative mosque-header">
        ...
        <div className="relative z-10">
          ...
          {nextPrayer && (
            <div className="glass-card rounded-2xl p-5 mt-4 border-l-4 border-l-amber-400/80">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-white/80 text-xs font-medium uppercase tracking-wider mb-1">Next Prayer</p>
                  <p className="text-3xl font-bold text-white drop-shadow-sm">{nextPrayer.name}</p>
                  <p className="text-white/70 text-sm mt-1">
                    at {formatPrayerTime(nextPrayer.time, use24HourFormat)}
                  </p>
                </div>
                <div className="text-right">
                  <p className="text-white/80 text-xs font-medium uppercase tracking-wider mb-1">Time Remaining</p>
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <p className={cn(
                      ...
                      {PRAYER_NAMES[prayer as keyof typeof PRAYER_NAMES]?.en || prayer}
                    </p>
